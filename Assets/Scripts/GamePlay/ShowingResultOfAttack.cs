...
public class ShowingResultOfAttack : MonoBehaviour 
{
  ...
  public void UpdateStatus(Character selectedCharacter,Character targetCharacter, int amountOfResult)
  {
    ...
    selectedData.GetChild (1).GetComponent<Text> ().text = selectedCharacter.name;
    selectedData.GetChild (2).GetComponent<Text> ().text = selectedCharacter.currentHP.ToString();
    selectedData.GetChild (3).GetComponent<Text> ().text = selectedCharacter.characterStatus.attack.ToString();

    targetData.GetChild (0).GetComponent<Image> ().sprite = Resources.Load<Sprite> ("PlayerPrefab/" + targetCharacter.name); 
    targetData.GetChild (1).GetComponent<Text> ().text = targetCharacter.name;
    targetData.GetChild (2).GetComponent<Text> ().text = targetCharacter.currentHP.ToString();
    targetData.GetChild (3).GetComponent<Text> ().text = targetCharacter.characterStatus.attack.ToString();
